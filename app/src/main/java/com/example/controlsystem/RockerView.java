...
public class RockerView extends View {
    ...
    private Paint mAreaBackgroundPaint;//可移动区域画笔
    private Paint mRockerPaint; //摇杆画笔

    private Path mPath1; //箭头path
    private Path mPath2;
    private Path mPath3;
    private Path mPath4;
    ...
    private OnPressDownListener mOnPressDownListener;
    ...
    private boolean press_flag = false; //按下标志
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (AREA_BACKGROUND_MODE_PIC == mAreaBackgroundMode || AREA_BACKGROUND_MODE_XML == mAreaBackgroundMode) {
            // 图片
            mAreaBackgroundPaint.setAlpha(100);
        ...
        }else if (ROCKER_BACKGROUND_MODE_XML == mRockerBackgroundMode) {
            // XML
            mRockerPaint.setAlpha(30);
            Rect src = new Rect(0, 0, mRockerBitmap.getWidth(), mRockerBitmap.getHeight());
            Rect dst = new Rect(mRockerPosition.x - mRockerRadius, mRockerPosition.y - mRockerRadius, mRockerPosition.x + mRockerRadius, mRockerPosition.y + mRockerRadius);
            canvas.drawBitmap(mRockerBitmap, src, dst, mRockerPaint);

            Log.d("time","ms:"+System.currentTimeMillis());

            //画箭头,上
            mPath1.moveTo(cx, 80);
            mPath1.lineTo(cx - 10, 140);
            mPath1.lineTo(cx, 100);
            mPath1.lineTo(cx + 10, 140);
            mPath1.close();

            //下
            mPath2.moveTo(cx, 2*cy-80);
            mPath2.lineTo(cx - 10, 2*cy-140);
            mPath2.lineTo(cx, 2*cy-100);
            mPath2.lineTo(cx + 10, 2*cy-140);
            mPath2.close();

            //左
            mPath3.moveTo(80, cy);
            mPath3.lineTo(140, cy - 10);
            mPath3.lineTo(100, cy);
            mPath3.lineTo(140, cy + 10);
            mPath3.close();

            //右
            mPath4.moveTo(2*cx-80, cy);
            mPath4.lineTo(2*cx-140, cy - 10);
            mPath4.lineTo(2*cx-100, cy);
            mPath4.lineTo(2*cx-140, cy + 10);
            mPath4.close();
            mAreaBackgroundPaint.setColor(Color.GRAY);
            mAreaBackgroundPaint.setAlpha(30);
            mAreaBackgroundPaint.setStrokeWidth(4);
            canvas.drawPath(mPath1, mAreaBackgroundPaint);
            canvas.drawPath(mPath2, mAreaBackgroundPaint);
            canvas.drawPath(mPath3, mAreaBackgroundPaint);
            canvas.drawPath(mPath4, mAreaBackgroundPaint);

            canvas.drawLine(cx, 100, cx, 160, mAreaBackgroundPaint);
            canvas.drawLine(cx, 2*cy-100, cx, 2*cy-160, mAreaBackgroundPaint);
            canvas.drawLine(100, cy, 160, cy, mAreaBackgroundPaint);
            canvas.drawLine(2*cx-100, cy, 2*cx-160, cy, mAreaBackgroundPaint);
            Log.d("time1","ms:"+System.currentTimeMillis());
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:// 按下
                if(!flag) {
                    mRockerPaint.setAlpha(20);
                    press_flag = !press_flag;
                    if(mOnPressDownListener != null){
                        mOnPressDownListener.press_flag(press_flag);
                    }
                    invalidate();
                }
                // 回调 开始
            ...
            case MotionEvent.ACTION_MOVE:// 移动
                ...
                if (null != mOnPositionChangeListener) {
                    mOnPositionChangeListener.mRockerPosition(mRockerPosition, lenXY);
                }
...
//                timer.schedule(task, 100);
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        // 两点距离
        lenXY = (float) Math.sqrt((double) (lenX * lenX + lenY * lenY));
        // 计算弧度
        ...
        if (lenXY + rockerRadius <= regionRadius) { // 触摸位置在可活动范围内
            ...
            float lenx = (float) (showPointX - centerPoint.x);
            float leny = (float) (showPointY - centerPoint.y);
            // 两点距离
            lenXY = (float) Math.sqrt((double) (lenx * lenx + leny * leny));
     ...
     * @param listener 回调接口
     */
    public void setOnPressDownListener(OnPressDownListener listener) {
        mOnPressDownListener = listener;
    }
    ...
    public interface OnPressDownListener {
         ...
         * @param press_flag 位置
         */
        void press_flag(boolean press_flag);
    ...
    public interface OnPositionChangeListener {
         ...
         * 摇动位置
         *
         * @param mRockerPosition 位置
         */
        void mRockerPosition(Point mRockerPosition, float lenXY);
